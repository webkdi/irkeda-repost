...
async function getPostOutOfArticle(articleText) {
    ...
    const message = `Ты копирайтер по соцсетям. Из длинного текста статьи напиши короткий пост для Телеграма.\nИспользуй простой молодежный язык. Добавляй немного емодзи и смайликов. Не делай весь текст из емодзи.  Пост не должен превышать 900 символов, включая пробелы.\nСначала напиши короткое название для поста. Потом с новой строки резюмируй следующую статью в коротком посте. Не добавляй ковычки для названия и основного теста. Статья:\n"""\n${articleText}\n"""`;
    if (token) {
